...
def _attach_produtos_to_pi(db: Session, pi: PI) -> None:
  """
  Monta um atributo transitório `pi.produtos` com a estrutura:
  [
    SimpleNamespace(
      id=produto_id,
      nome=produto_nome,
      descricao=produto_descricao,
      veiculacoes=[SimpleNamespace(...)]
    ), ...
  ]
  onde cada veic tem campo 'valor' = valor_liquido (preferência) ou valor_bruto.
  """
  # Pega todas as veiculações do PI com seus produtos
  ...
  agrup: Dict[int, SimpleNamespace] = {}
  for v, p in regs:
    if p.id not in agrup:
      agrup[p.id] = SimpleNamespace(
        id=p.id,
        nome=p.nome,
        descricao=getattr(p, "descricao", None),
        veiculacoes=[],
      )
    # proxy de veic para compat com schemas/rotas que esperam 'valor'
    v_proxy = SimpleNamespace(
      id=v.id,
    ...
    agrup[p.id].veiculacoes.append(v_proxy)

  # Atribui transitório
  pi.produtos = list(agrup.values())
