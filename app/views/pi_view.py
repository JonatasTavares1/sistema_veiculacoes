...
class PIView(ctk.CTkFrame):
    def __init__(self, master=None):
        ...
        self.checkbox_matriz = ctk.CTkCheckBox(
            ...
            command=self.alternar_combo_pi_matriz
        )
        self.checkbox_matriz.pack(pady=4, padx=20)

        # Combobox para selecionar PI Matriz existente (visível apenas se não for matriz)
        self.combo_pi_matriz = criar_combo(
            self.scrollable_frame,
            values=[pi.numero_pi for pi in listar_pis_matriz_ativos()],
    ...
    def alternar_combo_pi_matriz(self):
        if self.pi_matriz_var.get():  # É matriz → esconde combo
            self.combo_pi_matriz.pack_forget()
        else:  # Não é matriz → mostra combo
    ...
    def cadastrar_pi(self):
        try:
            ...
            numero_pi_matriz = "" if self.pi_matriz_var.get() else self.combo_pi_matriz.get()
            # ... (resto igual)
            # passe o campo `pi_matriz` normalmente na função criar_pi
            criar_pi(
                numero_pi=numero,
                pi_matriz=numero_pi_matriz,
                # ... demais campos
